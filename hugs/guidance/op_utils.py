...
class StableDiffusion(nn.Module):
    ...
    def train_step(
        ...
        if save_error_name is not None:
            ...
            plt.imsave('/home/zhouzhiyao/repo/ml-hugs/debug_error_map/' + save_error_name, error_map.detach().cpu().numpy(), cmap='jet')

            error_map = latents.float().detach()
            error_map = torch.sum(error_map, dim=1).squeeze() # 64, 64
            error_map = error_map - torch.min(error_map)
            error_map = error_map / torch.max(error_map) # 0-1
            plt.imsave('/home/zhouzhiyao/repo/ml-hugs/debug_error_map/latent/' + save_error_name, error_map.detach().cpu().numpy(), cmap='jet')
