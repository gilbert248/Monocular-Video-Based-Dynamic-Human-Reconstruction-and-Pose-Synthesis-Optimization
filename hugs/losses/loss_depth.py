...
class HumanSceneLoss_depth(nn.Module):
    def __init__(
        self,
        l_ssim_w=0.2,
        l_l1_w=0.8,
        ...
        super(HumanSceneLoss_depth, self).__init__()
        ...
        if add_depth:
            self.gt_depth = torch.from_numpy(np.load('/home/zhouzhiyao/repo/ml-hugs/depth_citron.npy').astype(np.float32)).to('cuda')
            # self.gt_depth = torch.from_numpy(np.load('/home/zhouzhiyao/repo/ml-hugs/depth_lab.npy').astype(np.float32)).to('cuda')
            # self.gt_depth = torch.from_numpy(np.load('/home/zhouzhiyao/repo/ml-hugs/depth_bike.npy').astype(np.float32)).to('cuda')
    ...
    def forward(
        ...
        if self.add_depth:
            pred_depth = render_pkg['depth'].squeeze()
        ...
        # if self.add_depth and epoch > 2000:
        if self.add_depth:
            gt_depth = self.gt_depth[dataset_idx]
            depth_loss = torch.abs(pred_depth - gt_depth).sum()  #L1 loss
            loss_dict['depth'] = self.depth_w * depth_loss
            extras_dict['gt_depth'] = gt_depth
            extras_dict['pred_depth'] = pred_depth

            bbox = data['bbox'].to(int)
            depth_human_loss = torch.abs(pred_depth[bbox[0]:bbox[2], bbox[1]:bbox[3]] - gt_depth[bbox[0]:bbox[2], bbox[1]:bbox[3]]).sum()
            loss_dict['depth_human'] = self.depth_w * depth_human_loss
            extras_dict['gt_human_depth'] = gt_depth[bbox[0]:bbox[2], bbox[1]:bbox[3]]
            extras_dict['pred_human_depth'] = pred_depth[bbox[0]:bbox[2], bbox[1]:bbox[3]]

        else:
            loss_dict['depth'] = torch.tensor(0.0).to('cuda')
            loss_dict['depth_human'] = torch.tensor(0.0).to('cuda')
