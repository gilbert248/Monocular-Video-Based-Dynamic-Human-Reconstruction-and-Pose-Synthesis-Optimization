...
import math
import torch
from diff_gaussian_rasterization_confidence import (
...
# from diff_gaussian_rasterization_obj import GaussianRasterizationSettings as GaussianRasterizationSettings_obj
# from diff_gaussian_rasterization_obj import GaussianRasterizer as GaussianRasterizer_obj
...
def render_human_scene(
    ...
    if render_mode == 'human_scene':
        ...
        # scene_cls = torch.zeros(scene_gs_out['xyz'].shape[0])     #semantic
        # human_cls = human_gs_out['gs_cls']#shape:[human_gs.n_gs]
        # gs_cls = torch.cat([human_cls, scene_cls])
    ...
    if render_human_separate and render_mode == 'human_scene':
        ...
        # render_pkg['human_depth'] = render_human_pkg['depth']
        # render_pkg['human_alpha'] = render_human_pkg['alpha']

        ############render depth normalization#################
        # human_depth = render_pkg['human_depth'].squeeze(0)
        # x_max = human_depth.max()
        # x_min = human_depth.min()
        # normalized_depth = (human_depth - x_min) / (x_max - x_min)
        # render_pkg['human_depth'] = normalized_depth.unsqueeze(0)
        #####################################################
        # N_cls =  23
        # H, W = render_pkg['human_alpha'].shape[1], render_pkg['human_alpha'].shape[2]
        # semantic_graph = torch.zeros(N_cls-1, H, W).to('cuda')     #shape: [N_cls, H, W]
        # for cls_i in range(N_cls-1): 
        #     mask = (cls_i==gs_cls)
            
        #     feats = human_gs_out['shs'][mask]
        #     means3D = human_gs_out['xyz'][mask]
        #     opacity = human_gs_out['opacity'][mask]
        #     scales = human_gs_out['scales'][mask]
        #     rotations = human_gs_out['rotq'][mask]
        #     active_sh_degree = human_gs_out['active_sh_degree']
        ...
        #         active_sh_degree=active_sh_degree,
        #         )
        #     semantic_graph[cls_i] = render_pkg_i['alpha'].squeeze(0)

        # semantic_graph = torch.cat([1 - render_pkg['human_alpha'], semantic_graph], dim=0)#background, cls==0


    if render_mode == 'human':
        render_pkg['human_visibility_filter'] = render_pkg['visibility_filter']
        render_pkg['human_radii'] = render_pkg['radii']
        # render_pkg['human_depth'] = render_pkg['depth']
...
def render(means3D, feats, opacity, scales, rotations, data, scaling_modifier=1.0, bg_color=None, active_sh_degree=0):
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        viewmatrix=data['world_view_transform'],    #extrinsic
        projmatrix=data['full_proj_transform'],     #intrinsic
        
        sh_degree=active_sh_degree,
        campos=data['camera_center'],
        prefiltered=False,
        debug=False,
        # confidence=torch.ones_like(torch.tensor(means3D.shape[0])).to('cuda')
        # confidence=torch.ones(means3D.shape[0], device="cuda")
        confidence=torch.ones(means3D.shape[0], device="cuda").unsqueeze(-1)
        # antialiasing = True
    )
    ...
    return {
        ...
        "radii": radii,         ##if this pixel is visible, its radii is > 0
        # "depth": depth,
        # "alpha": alpha,
    }
