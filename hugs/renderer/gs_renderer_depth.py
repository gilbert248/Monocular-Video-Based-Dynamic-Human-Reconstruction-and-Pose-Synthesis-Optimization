...
import math
import torch
from diff_gaussian_rasterization_depth1 import (
    GaussianRasterizationSettings, 
    GaussianRasterizer
)

def render_human_scene_depth(
    data, 
    human_gs_out,
    scene_gs_out,
    bg_color, 
...
def render_depth(means3D, feats, opacity, scales, rotations, data, scaling_modifier=1.0, bg_color=None, active_sh_degree=0):
    if bg_color is None:
    ...
    rendered_image, radii, depth, weight_map= rasterizer(
        means3D=means3D,
        means2D=means2D,
        shs=shs,
        opacities=opacity,
    ...
    max, min = depth.max(), depth.min()
    depth_out = (depth - min) / (max - min)
